...
#include <algorithm>
#include <chrono>
#include "get_nms_before_boxes.h"

#define USE_GPU_PREPROCESS 1

YOLO::YOLO(const std::string& OnnxFilePath, const std::string& SaveTrtFilePath, int BatchSize, int InputChannel, int InputImageWidth, int InputImageHeight)
{
...
void YOLO::Inference(cv::Mat& SrcImage)
{
...
void YOLO::PrepareImage(cv::Mat& SrcImage, std::vector<float>& PreprocessResult)
{
...
void YOLO::PrepareImage(cv::Mat& SrcImage, void* InputBuffer)
{
#if USE_GPU_PREPROCESS
